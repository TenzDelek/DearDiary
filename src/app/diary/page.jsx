...
export default function Diary() {
  ...
  const handleModalSubmit = async (e) => {
    e.preventDefault();
    // checking if the title and content in the modal is empty
    if (currentDiary.title === "") {
      setErrorModalMessage((prev) => ({
        ...prev,
        modalTitle: "Please enter the title",
      }));
      return;
    }
  ...
  const handleSubmit = (event) => {
    ...
    // checking if the title and content is empty
    if (form.title === "") {
      setErrorMessage((prev) => ({ ...prev, title: "Please enter the title" }));
      return;
    }
    if (form.content === "") {
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid w-full gap-4">
              ...
              {errorMessage.content && (
                <p className="text-red-500 text-sm">{errorMessage.content}</p>
              )}
            </div>
            {loading ? (
              <Button
                ...
                <div className="grid w-full gap-4">
                  ...
                  <div className="flex flex-col space-y-1.5">
                    ...
                    <Input
                      ...
                      className="bg-input text-foreground placeholder-muted-foreground border border-border rounded-md p-2 focus:ring focus:ring-primary"
                    />
                    {errorModalMessage.modalContent && (
                      <p className="text-red-500 text-sm">
                        {errorModalMessage.modalContent}
                      </p>
                    )}
